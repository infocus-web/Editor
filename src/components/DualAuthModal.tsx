...
import { X, Sparkles, Key, Mail, User, Check, ShieldCheck, LogIn, LogOut, CheckCircle2 } from 'lucide-react';
...
export const DualAuthModal: React.FC<DualAuthModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'gemini' | 'chatgpt'>('chatgpt');
  ...
  const [chatgptName, setChatgptName] = useState(authState.chatgpt.name || 'Usuario ChatGPT');
  ...
  useEffect(() => {
    if (isOpen) {
      setGeminiEmail(authState.gemini.email || 'alderpol@gmail.com');
      setGeminiName(authState.gemini.name || 'Google User');
      setGeminiKey(authState.gemini.apiKey || '');

      setChatgptEmail(authState.chatgpt.email || 'alderpol@openai.com');
      setChatgptName(authState.chatgpt.name || 'Usuario ChatGPT');
      setChatgptKey(authState.chatgpt.apiKey || '');
      setJustSaved(false);
    }
  }, [isOpen, authState]);
  ...
  const handleGeminiLogin = (e: React.FormEvent) => {
    ...
    const updated: DualAuthState = {
      ...authState,
      gemini: {
        email: geminiEmail.trim() || 'alderpol@gmail.com',
        name: geminiName.trim() || 'Usuario Google',
        isLoggedIn: true,
  ...
  const handleChatgptLogin = (e: React.FormEvent) => {
    ...
    const updated: DualAuthState = {
      ...authState,
      chatgpt: {
        email: chatgptEmail.trim() || 'alderpol@openai.com',
        name: chatgptName.trim() || 'Usuario ChatGPT',
        isLoggedIn: true,
        apiKey: chatgptKey.trim(),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg rounded-2xl bg-neutral-900 border border-neutral-800 shadow-2xl overflow-hidden">
        ...
        {justSaved && (
          <div className="mx-6 mt-4 p-3 rounded-xl bg-emerald-950/60 border border-emerald-500/50 flex items-center space-x-2 text-emerald-300 text-xs animate-fade-in">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="font-semibold">¡Cambios guardados con éxito! Cerrando ventana...</span>
          </div>
        )}
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-neutral-400">
                      {authState.chatgpt.isLoggedIn
                        ...
                        : 'Modo asistido activo (puedes chatear sin API key)'}
                    </p>
                  </div>
                </div>
                {authState.chatgpt.isLoggedIn && (
              ...
              <form onSubmit={handleChatgptLogin} className="space-y-3.5">
                <div>
                  <label className="block text-xs font-medium text-neutral-300 mb-1 flex items-center space-x-1">
                    ...
                    <span>Correo o Usuario OpenAI</span>
                  </label>
                  <input
                    type="text"
                    value={chatgptEmail}
                ...
                <div>
                  <label className="block text-xs font-medium text-neutral-300 mb-1 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-emerald-400 font-medium">No obligatoria</span>
                  </label>
                  <input
                    type="password"
                    value={chatgptKey}
                    onChange={(e) => setChatgptKey(e.target.value)}
                    placeholder="sk-proj-... (déjalo vacío si no tienes)"
                  ...
                  <p className="text-[10px] text-neutral-400 mt-1">
                    Si no tienes API Key, déjalo en blanco. El chat responderá con el asistente inteligente integrado.
                  </p>
                </div>

                <div className="pt-2 space-y-2">
                  <button
                    id="save-chatgpt-auth-btn"
                    type="submit"
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="w-full py-2 px-4 rounded-xl text-xs text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800/60 transition-colors"
                  >
                    Cancelar / Salir al Chat
                  </button>
                </div>
              </form>
            </div>
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-neutral-400">
                      {authState.gemini.isLoggedIn
                        ...
                        : 'Conexión activa con el servidor de Gemini'}
                    </p>
                  </div>
                </div>
                {authState.gemini.isLoggedIn && (
              ...
              <form onSubmit={handleGeminiLogin} className="space-y-3.5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="alderpol@gmail.com"
                ...
                <div>
                  <label className="block text-xs font-medium text-neutral-300 mb-1 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-emerald-400">Ya activo en servidor</span>
                  </label>
                  <input
                    type="password"
                    value={geminiKey}
        ...
        <div className="px-6 py-3 bg-neutral-950 border-t border-neutral-800 flex items-center justify-between text-[11px] text-neutral-400">
          <span className="flex items-center space-x-1">
            ...
            <span>Tus datos se guardan en el navegador</span>
          </span>
          <button
            type="button"
            onClick={onClose}
